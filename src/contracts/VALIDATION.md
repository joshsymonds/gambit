...
the user's `~/.claude/CLAUDE.md`, which already carries that exact rule — baselines cited it by name
...
(`executing-plans`), which appears nowhere in `CLAUDE.md`. Scenario: a dispatched worker silent for
