...
- **Coverage floor — one happy-path test is never enough.** Before reporting, map every success criterion and every distinct behavior the brief names — each error path, boundary, state transition, and enumerated scenario — to at least one assertion that fails if that behavior breaks. Your report MUST include that mapping (criterion → test). If the brief enumerates N scenarios, cases, or codes, your tests cover all N or your report names each gap explicitly. A behavior you cannot map to a test is a concern: return DONE_WITH_CONCERNS naming it, never bare DONE.
...
- **DONE** — all success criteria met, tests green, RED/GREEN evidence included, no concerns. Report: the exact changed-file list including untracked/binary files, deletions, symlinks, and mode changes; functions changed; the test command + one-line result; the coverage mapping (criterion → test).
