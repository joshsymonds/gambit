...
3. **Run one combined gate.** Run the manifest's declared wave/component gate exactly once after every worker commit is combined on the detached candidate HEAD. A nonzero gate or any tracked, staged, or non-ignored untracked integration artifact fails closed. Release acceptance remains a separate final-system claim and is not the manifest gate unless the approved Validation Strategy explicitly says so.
