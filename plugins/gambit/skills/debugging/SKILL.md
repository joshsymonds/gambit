---
name: debugging
description: "Investigates a bug to its root cause with evidence and a failing test, then hands the understanding off — it does not implement the fix. Use when a test is failing, when a bug is reported, when behavior is unexpected or intermittent, when a build or integration step fails, or when a flaky test keeps resurfacing. Especially when \"the fix seems obvious\", when multiple previous fixes haven't stuck, or when under time pressure to ship."
---
...
For bounded codebase investigation, Glob `**/codex-contracts/scout.md`, dispatch the `scout` role using
`explorer`, and tell it to Read `codex-contracts/scout.md` first.
