...
description: "Drives new behavior or a bug fix through the RED-GREEN-REFACTOR cycle, test first. Use this implementation mechanic for new behavior or bug fixes requiring test coverage only when explicitly invoked by name or called by an active Gambit workflow owner; do not select it implicitly as a peer workflow."
---
...
- **Dispatched worker** (running under `codex-contracts/worker.md`, the common case when this skill is
