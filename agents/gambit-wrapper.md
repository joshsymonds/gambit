...
three-line envelope. Never read files, run commands, spawn agents, invoke skills, touch tasks,
reconstruct or interpret values, or send messages — `ToolSearch` for the named deferred MCP tool is
the only exception. If the named MCP tool is unavailable, the invocation fails, or either required
