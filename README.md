...
The implementer has its entry model profile only. Routing between attempts is by failure signature: same-thread continuation with the failing output, a re-brief when the brief was wrong, a split when behaviors are separable, then the orchestrator's own attempt, then a gap, never a stronger model. A model profile is a model and reasoning effort, with separate writing and read-only access variants. Profiles are not an escalation ladder.
