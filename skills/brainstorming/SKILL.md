...
7. **Done:** exact commands for each task's fast check and the integrated candidate's full gate, including required setup, and the effort ceiling `max_efforts`.
...
In conversation, present the acceptance sheet and the complete contract for acceptance and settle requested changes here. With a goal file, first check that every assumption the research produced, including each fact resolved after a `NOT FOUND` and each question you answered yourself, has its Decision Log entry, and log any missing one; then accept the contract yourself on that file's behalf. Record acceptance and freeze Intent, Premises with their survival clauses, and Requirements. Later changes of assessment belong in the Decision Log, leaving those clauses intact. The contract alone authorizes the work that follows.

At acceptance, and in addition to the record-task-state operation, write the epic's record directory `~/.gambit/<repository-id>/<epic-slug>/` as `skills/executing-plans/references/record.md` specifies: the frozen eight-section contract to `epic.md`, every Decision Log entry to `decisions.md` in its append-only line format, and the head to `state.json`, carrying the accepted `max_efforts` and `efforts_admitted` at zero. Derive repository-id from the tree by that reference's rule, never from a workspace name. The record carries this epic for a reader holding no transcript.

## Acceptance sheet

Before presenting the contract for acceptance, emit the acceptance sheet from `TEMPLATES.md`. Trace every Requirement, Approach mechanism, Done command, and Release action to a sentence of the person's request or goal file; whatever traces to nothing is PROPOSED. Give each PROPOSED item one line with its estimated cost: new files, new infrastructure such as a machine, service, harness, script, or corpus, external systems touched, and Done runtime. Collapse traced items to counts. Close with the size line: the Requirement count, the effort estimate with its one-line reason, the proposed `max_efforts`, and the PROPOSED count. The estimate is a judgment of decomposition and dependency shown apart from the maximum; the maximum bounds how many efforts the run may admit, not its time or spend. The sheet is at most forty lines with at most thirty words per PROPOSED line; when the PROPOSED items would overflow it, continue them on a numbered second sheet that closes with the same size line, so every item stays listed.

In conversation, the person decides each PROPOSED item and the maximum individually before accepting the contract, and each decision is a Decision Log entry. Striking an item removes it along with the evidence and Done commands that depended on it. With a goal file, decide each PROPOSED item and the maximum yourself from the goal and research, log each decision with its reason, and continue. The accepted maximum is written into the contract's Done section, which freezes with the contract, and becomes `max_efforts` in the head at acceptance; the head copies that value and never changes it. Execution admits no effort beyond it and treats infrastructure outside the accepted Approach and accepted proposals as unauthorized, so the sheet is where a mechanism the person never asked for is caught.
